...
function _footer() {
  ...
  <div class="doc-footer">
    ...
    <div class="footer-sign">
      ...
      <div class="sign-title">المشرف الإداري — التوقيع</div>
    </div>
    <div class="footer-sign">
      <div class="sign-line"></div>
      <div class="sign-name">الداعية د. محمد دحروج</div>
      <div class="sign-title">المشرف الديني — التوقيع</div>
    </div>
    <div class="footer-sign">
      <div class="sign-line"></div>
...
//  quoteData : row from quotations table (with customers join)
//  items     : rows from quotation_items ordered by sort_order
// ══════════════════════════════════════════════════════════
function printQuotation(q, items) {
  ...
  const itemRows = (items||[]).map((it, i) => {
    const icon = {
      hotel:'🏨', flight:'✈️', transport:'🚌', guide:'🧭',
      visa:'📋', excursion:'🗺️', meal:'🍽️', ticket:'🎫', other:'📦'
    }[it.item_type] || '📦';

    let dateRange = '';
    if (it.date_from || it.date_to) {
      dateRange = `<br><span style="font-size:11px;color:#6b7280">${fmtDate(it.date_from)}${it.date_to && it.date_to !== it.date_from ? ' — ' + fmtDate(it.date_to) : ''}</span>`;
    }
    const nights = it.nights ? `<br><span style="font-size:11px;color:#6b7280">${it.nights} ليلة</span>` : '';

    return `<tr>
      <td style="text-align:center;color:#6b7280">${i+1}</td>
      <td style="text-align:center;font-size:16px">${icon}</td>
      <td><strong>${it.description||'—'}</strong>${dateRange}${nights}</td>
      <td style="text-align:center;color:#374151">${ITEM_TYPE_AR[it.item_type]||it.item_type}</td>
      <td style="text-align:center">${it.quantity||1} ${it.unit_label||''}</td>
      <td style="text-align:center;direction:ltr">${fmt(it.unit_price)} ${curr}</td>
      <td style="text-align:center;font-weight:700;color:#1B5E20;direction:ltr">${fmt(it.total_price)} ${curr}</td>
    </tr>`;
  }).join('');

  // ── Totals ─────────────────────────────────────────────
  const discountRow = Number(q.discount_amount||0) > 0
    ? `<tr><td colspan="6" style="text-align:left;padding:6px 10px;color:#ca8a04">خصم (${q.discount_pct||0}%)</td><td style="text-align:center;direction:ltr;color:#ca8a04;padding:6px 10px">- ${fmt(q.discount_amount)} ${curr}</td></tr>`
    : '';
  const taxRow = Number(q.tax_amount||0) > 0
    ? `<tr><td colspan="6" style="text-align:left;padding:6px 10px;color:#374151">ضريبة (${q.tax_pct||0}%)</td><td style="text-align:center;direction:ltr;padding:6px 10px">${fmt(q.tax_amount)} ${curr}</td></tr>`
    : '';

  // ── Inclusions / Exclusions ────────────────────────────
  const inclHtml = q.included_services
    ? `<div class="section" style="margin-top:0">
        <div class="section-title"><i>✅</i> يشمل العرض</div>
        <div style="font-size:13px;color:#374151;white-space:pre-line;padding:0 4px">${q.included_services}</div>
       </div>`
    : '';

  const exclHtml = q.excluded_services
    ? `<div class="section" style="margin-top:0">
        <div class="section-title"><i>❌</i> لا يشمل العرض</div>
        <div style="font-size:13px;color:#374151;white-space:pre-line;padding:0 4px">${q.excluded_services}</div>
       </div>`
    : '';

  // ── Notes ──────────────────────────────────────────────
  const notesHtml = q.client_notes
    ? `<div class="section" style="margin-top:0">
        <div class="section-title"><i>📝</i> ملاحظات</div>
        <div style="font-size:13px;color:#374151;white-space:pre-line;padding:0 4px">${q.client_notes}</div>
       </div>`
    : '';

  // ── Terms ──────────────────────────────────────────────
  const termsHtml = (q.payment_terms || q.cancellation_policy)
    ? `<div class="section" style="margin-top:0">
        <div class="section-title"><i>📜</i> شروط وأحكام</div>
        <table style="width:100%;border-collapse:collapse;font-size:12px;">
          ${q.payment_terms ? `<tr><td style="padding:4px 8px;font-weight:700;width:140px;color:#1B5E20">شروط الدفع</td><td style="padding:4px 8px">${q.payment_terms}</td></tr>` : ''}
          ${q.cancellation_policy ? `<tr><td style="padding:4px 8px;font-weight:700;color:#1B5E20">سياسة الإلغاء</td><td style="padding:4px 8px">${q.cancellation_policy}</td></tr>` : ''}
        </table>
       </div>`
    : '';

  // ── Validity badge ─────────────────────────────────────
  const validTill = q.validity_date
    ? `<span style="background:#fef9c3;color:#854d0e;padding:2px 10px;border-radius:20px;font-size:12px;font-weight:700">صالح حتى: ${fmtDate(q.validity_date)}</span>`
    : '';
  ...
  const customerName = q.customers?.full_name || q.customer_name || '';
...
.items-table th{background:#1B5E20;color:#fff;padding:8px 10px;font-weight:700;text-align:right;}
.items-table td{padding:8px 10px;border-bottom:1px solid #f3f4f6;vertical-align:top;}
.items-table tr:nth-child(even) td{background:#f9fafb;}
.totals-table{width:100%;border-collapse:collapse;margin-top:4px;}
.grand-total{background:#f0fdf4;font-weight:900;font-size:16px;border-top:2px solid #1B5E20;}
.grand-total td{padding:10px 10px;}
</style>
</head><body>
<button id="printBtn" onclick="window.print()">🖨️ طباعة / حفظ كـ PDF</button>
<div class="page-wrap">
  ...
  ${_header('عـرض سـعـر', `
    <span>رقم العرض: <strong style="color:#1B5E20">${q.quote_number||'—'}</strong></span>
    &nbsp;|&nbsp;
  ...
  <div class="section" style="margin-top:0">
    <table style="width:100%;border-collapse:collapse;font-size:13px;">
      <tr>
        <td style="padding:5px 8px;font-weight:700;color:#1B5E20;width:140px">اسم العميل</td>
        <td style="padding:5px 8px">${customerName||'—'}</td>
        <td style="padding:5px 8px;font-weight:700;color:#1B5E20;width:140px">الوجهة</td>
        <td style="padding:5px 8px">${q.destination||'—'}</td>
      </tr>
      <tr>
        <td style="padding:5px 8px;font-weight:700;color:#1B5E20">تاريخ السفر</td>
        <td style="padding:5px 8px">${q.travel_date_from ? fmtDate(q.travel_date_from) + (q.travel_date_to ? ' — ' + fmtDate(q.travel_date_to) : '') : '—'}</td>
        <td style="padding:5px 8px;font-weight:700;color:#1B5E20">عدد المسافرين</td>
        <td style="padding:5px 8px">${paxStr||'—'}</td>
      </tr>
      <tr>
        <td style="padding:5px 8px;font-weight:700;color:#1B5E20">العملة</td>
        <td style="padding:5px 8px">${curr}</td>
        <td style="padding:5px 8px;font-weight:700;color:#1B5E20">إصدار</td>
        <td style="padding:5px 8px">v${q.version||1}</td>
      </tr>
    </table>
  </div>
  ...
  <!-- Items -->
  <div class="section" style="margin-top:0">
    <div class="section-title"><i>📋</i> بنود الخدمات</div>
    <table class="items-table">
      <thead>
        <tr>
          ...
          <th style="width:110px;text-align:center">السعر للوحدة</th>
          <th style="width:120px;text-align:center">الإجمالي</th>
        </tr>
      </thead>
      <tbody>
        ${itemRows || '<tr><td colspan="7" style="text-align:center;color:#9ca3af;padding:16px">لا توجد بنود</td></tr>'}
      </tbody>
    </table>

    <!-- Totals -->
    <table class="totals-table" style="margin-top:0">
      <tr><td colspan="6" style="text-align:left;padding:6px 10px;color:#374151">المجموع قبل الخصم</td><td style="text-align:center;direction:ltr;padding:6px 10px">${fmt(q.subtotal)} ${curr}</td></tr>
      ${discountRow}
      ${taxRow}
      <tr class="grand-total"><td colspan="6" style="text-align:left;color:#1B5E20">الإجمالي النهائي</td><td style="text-align:center;direction:ltr;color:#1B5E20">${fmt(q.total_amount)} ${curr}</td></tr>
    </table>
  </div>
